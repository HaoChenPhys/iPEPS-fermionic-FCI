...
box = [0.205, 0.60, 0.75, 0.33]   # tune these
gap = 0.015                       # vertical gap between the two inset axes
h_each = (box[3] - gap) / 2
...
E0_by_D1 = {D: (E0, sE0) for D, (E0, sE0) in E0_by_D.items() if D <= 6}
plot_E0_vs_D(E0_by_D1, marker="x", ax=ax_in_top, markersize=7, markeredgewidth=2.5)
plot_E0_vs_D(E0_by_D, marker="x", ax=ax_in_bot, markersize=7, markeredgewidth=2.5)

# choose y-windows (example: break around ~ -0.858)
ax_in_top.set_ylim(-0.859, -0.8420)
ax_in_bot.set_ylim(-0.860, -0.8588)
...
ax_in_bot.set_xticks([4,5,6,7,8,9])

handles, labels = ax_main.get_legend_handles_labels()

# D=4,5,6 (first three curves)
leg_top = ax_main.legend(
    handles[:3], labels[:3],
    frameon=False,
    loc="upper right",
    bbox_to_anchor=(0.94, 0.92),   # slight padding inside corner
    borderaxespad=0.0,
    handletextpad=0.2,
    labelspacing=0.3,
)
ax_main.add_artist(leg_top)  # keep this legend when adding the next one
leg_top.set_zorder(1000)

# D=7,8,9 (last three curves)
leg_bot = ax_main.legend(
    handles[3:], labels[3:],
    frameon=False,
    loc="lower left",
    bbox_to_anchor=(0.18, 0.59),   # slight padding inside corner
    borderaxespad=0.0,
    handletextpad=0.2,
    labelspacing=0.3,
)
leg_bot.set_zorder(1000)
ax = axes[1]
...
plt.savefig("figs/E_scaling.pdf", bbox_inches="tight")

fig, ax = plt.subplots(figsize=(4, 4))
sc = plot_densities(ns_data[0], ax=ax, fig=fig, colorbar=False)
plt.savefig(f"figs/density_D_{Ds[0]}.pdf", bbox_inches="tight")

fig, ax = plt.subplots(figsize=(4, 4))
sc = plot_densities(ns_data[-1], ax=ax, fig=fig)
plt.savefig(f"figs/density_D_{Ds[-1]}.pdf", bbox_inches="tight")
